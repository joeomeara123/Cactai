...
export class DatabaseClient {
  ...
  async createUserProfile(user: { id: string; email: string; full_name?: string; avatar_url?: string }): Promise<User | null> {
    const { data, error } = await this.supabase
      .from('user_profiles')
      .insert({
        id: user.id,
  ...
  async getOrCreateUserProfile(user: { id: string; email: string; full_name?: string; avatar_url?: string }): Promise<User | null> {
